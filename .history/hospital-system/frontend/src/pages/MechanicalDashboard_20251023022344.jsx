            ...
            <div className="content">
                ...
                <main>
                   ...
                   {currentView === 'work-order-details' && selectedWorkOrder && (
                       <>
                           <div className="page-header">
                               <button className="btn btn-secondary" onClick={() => setCurrentView('work-orders')}>
                                   &lt; Back to Work Orders
                               </button>
                               <h2>Work Order #{selectedWorkOrder.id}</h2>
                           </div>

                           <div className="work-order-details-panel">
                               <div className="details-section">
                                   <h3>{selectedWorkOrder.title}</h3>
                                   <p><strong>Description:</strong> {selectedWorkOrder.description}</p>
                                   <div className="details-grid">
                                       <div><strong>Status:</strong> <span className={`status-badge status-${selectedWorkOrder.status}`}>{selectedWorkOrder.status.replace('_', ' ')}</span></div>
                                       <div><strong>Priority:</strong> {selectedWorkOrder.priority}</div>
                                       <div><strong>Location:</strong> {selectedWorkOrder.location}</div>
                                       <div><strong>Category:</strong> {selectedWorkOrder.category}</div>
                                       <div><strong>Assigned To:</strong> {selectedWorkOrder.assigned_to || 'Unassigned'}</div>
                                       <div><strong>Estimated Hours:</strong> {selectedWorkOrder.estimated_hours || 'TBD'}</div>
                                   </div>
                               </div>
                                   ...
                                   <div className="add-comment">
                                       ...
                                       <button
                                           ...
                                           onClick={async () => {
                                               if (newComment.trim()) {
                                                   try {
                                                       await api.post(`/work-order/${selectedWorkOrder.id}/comment`, { comment: newComment });
